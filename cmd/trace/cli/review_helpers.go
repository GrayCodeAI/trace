...
// newReviewAttachCmd is a thin wrapper around `trace attach --review`. It
// shares all wiring with runAttach; only the UX surface differs, letting
// users discover review-attach through `trace review` in help output.
//
...
(without 'trace review') and want the review metadata attached after
the fact.
