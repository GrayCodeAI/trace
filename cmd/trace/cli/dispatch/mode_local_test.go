...
func TestLocalMode_ImplicitCurrentBranchUsesHEADReachability(t *testing.T) {
	...
	cpID := testCheckpointID
	testutil.GitCheckoutNewBranch(t, dir, "trace-dispatch")
	...
	testutil.GitCheckoutNewBranch(t, dir, "trace-dispatch-codex")
	...
	got, err := Run(context.Background(), Options{
		Mode:                  ModeLocal,
		Since:                 "7d",
		Branches:              []string{"trace-dispatch-codex"},
...
func TestReachableCheckpointIDsInRange_LimitsLogToWindowAndCheckpointTrailers(t *testing.T) {
	...
	if !strings.Contains(args, "--grep") || !strings.Contains(args, "Trace-Checkpoint:") {
