...
func TestInstallHooks_CreatesConfig(t *testing.T) {
	...
	assertHookCommand(t, hooksFile.Hooks.SessionStart, agentpkg.WrapProductionJSONWarningHookCommand("trace hooks codex session-start", agentpkg.WarningFormatSingleLine), "SessionStart")
	assertHookCommand(t, hooksFile.Hooks.UserPromptSubmit, agentpkg.WrapProductionSilentHookCommand("trace hooks codex user-prompt-submit"), "UserPromptSubmit")
	assertHookCommand(t, hooksFile.Hooks.Stop, agentpkg.WrapProductionSilentHookCommand("trace hooks codex stop"), "Stop")
...
func TestInstallHooks_LocalDev(t *testing.T) {
	...
	data, err := os.ReadFile(hooksPath)
	require.NoError(t, err)
	require.Contains(t, string(data), `go run \"$(git rev-parse --show-toplevel)\"/cmd/trace/main.go hooks codex session-start`)
}
...
func TestUninstallHooks_PreservesUserHookContainingTraceSubstring(t *testing.T) {
	...
	require.NotContains(t, string(data), "trace hooks codex stop")
}
...
func TestInstallHooks_PreservesExistingHooksJSON(t *testing.T) {
	...
	require.Contains(t, string(data), "trace hooks codex stop")
}
