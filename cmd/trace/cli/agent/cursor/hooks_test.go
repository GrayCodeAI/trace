...
func TestInstallHooks_FreshInstall(t *testing.T) {
	...
	// Verify commands
	assertEntryCommand(t, hooksFile.Hooks.Stop, agent.WrapProductionSilentHookCommand("trace hooks cursor stop"))
	assertEntryCommand(t, hooksFile.Hooks.SessionStart, agent.WrapProductionSilentHookCommand("trace hooks cursor session-start"))
	assertEntryCommand(t, hooksFile.Hooks.BeforeSubmitPrompt, agent.WrapProductionSilentHookCommand("trace hooks cursor before-submit-prompt"))
	assertEntryCommand(t, hooksFile.Hooks.PreCompact, agent.WrapProductionSilentHookCommand("trace hooks cursor pre-compact"))
	assertEntryCommand(t, hooksFile.Hooks.SubagentStart, agent.WrapProductionSilentHookCommand("trace hooks cursor subagent-start"))
	assertEntryCommand(t, hooksFile.Hooks.SubagentStop, agent.WrapProductionSilentHookCommand("trace hooks cursor subagent-stop"))
}
...
func TestInstallHooks_LocalDev(t *testing.T) {
	...
	hooksFile := readHooksFile(t, tempDir)
	assertEntryCommand(t, hooksFile.Hooks.Stop, `go run "$(git rev-parse --show-toplevel)"/cmd/trace/main.go hooks cursor stop`)
}
...
func TestInstallHooks_PreservesUnknownFields(t *testing.T) {
  ...
  "hooks": {
	...
	assertEntryCommand(t, stopHooks, agent.WrapProductionSilentHookCommand("trace hooks cursor stop"))
}
