...
// Cursor hook names - these become subcommands under `trace hooks cursor`
const (
...
func (c *CursorAgent) InstallHooks(ctx context.Context, localDev bool, force bool) (int, error) {
	...
	if localDev {
		cmdPrefix = `go run "$(git rev-parse --show-toplevel)"/cmd/trace/main.go hooks cursor `
	} else {
		cmdPrefix = "trace hooks cursor "
	}
