...
// Gemini CLI hook names - these become subcommands under `trace hooks gemini`
const (
...
func (g *GeminiCLIAgent) InstallHooks(ctx context.Context, localDev bool, force bool) (int, error) {
	...
	if localDev {
		cmdPrefix = `go run "$(git rev-parse --show-toplevel)"/cmd/trace/main.go hooks gemini `
	} else {
		cmdPrefix = "trace hooks gemini "
	}
