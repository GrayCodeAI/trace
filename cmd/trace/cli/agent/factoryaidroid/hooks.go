...
// Factory AI Droid hook names - these become subcommands under `trace hooks factoryai-droid`
const (
...
func (f *FactoryAIDroidAgent) InstallHooks(ctx context.Context, localDev bool, force bool) (int, error) {
	...
	localDevPrefix := `go run "$(git rev-parse --show-toplevel)"/cmd/trace/main.go hooks factoryai-droid `
	if localDev {
		...
		sessionStartCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid session-start")
		sessionEndCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid session-end")
		stopCmd = agent.WrapProductionPlainTextWarningHookCommand("trace hooks factoryai-droid stop", agent.WarningFormatSingleLine)
		userPromptSubmitCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid user-prompt-submit")
		preTaskCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid pre-tool-use")
		postTaskCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid post-tool-use")
		preCompactCmd = agent.WrapProductionSilentHookCommand("trace hooks factoryai-droid pre-compact")
	}
