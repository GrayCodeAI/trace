...
// setupDiscoveryDir creates a temp directory containing a mock trace-agent-<name> binary.
...
func enableExternalAgents(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(traceDir, "settings.json"), []byte(`{"enabled":true,"external_agents":true}`), 0o644); err != nil {
...
func TestDiscoverAndRegister_SkipsWhenDisabled(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(traceDir, "settings.json"), []byte(`{"enabled":true}`), 0o644); err != nil {
		t.Fatalf("write settings: %v", err)
	}
	t.Chdir(tmpDir)
