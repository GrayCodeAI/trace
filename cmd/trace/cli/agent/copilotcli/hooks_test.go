...
func TestInstallHooks_FreshInstall(t *testing.T) {
	...
	assertEntryBash(t, hooksFile.Hooks.UserPromptSubmitted, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli user-prompt-submitted"))
	assertEntryBash(t, hooksFile.Hooks.SessionStart, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli session-start"))
	assertEntryBash(t, hooksFile.Hooks.AgentStop, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli agent-stop"))
	assertEntryBash(t, hooksFile.Hooks.SessionEnd, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli session-end"))
	assertEntryBash(t, hooksFile.Hooks.SubagentStop, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli subagent-stop"))
	assertEntryBash(t, hooksFile.Hooks.PreToolUse, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli pre-tool-use"))
	assertEntryBash(t, hooksFile.Hooks.PostToolUse, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli post-tool-use"))
	assertEntryBash(t, hooksFile.Hooks.ErrorOccurred, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli error-occurred"))
...
func TestInstallHooks_LocalDev(t *testing.T) {
	...
	hooksFile := readHooksFile(t, tempDir)
	assertEntryBash(t, hooksFile.Hooks.AgentStop, `go run "$(git rev-parse --show-toplevel)"/cmd/trace/main.go hooks copilot-cli agent-stop`)
}
...
func TestInstallHooks_PreservesUnknownFields(t *testing.T) {
  ...
  "hooks": {
	...
	assertEntryBash(t, agentStopHooks, agent.WrapProductionSilentHookCommand("trace hooks copilot-cli agent-stop"))
}
