...
// metadataDenyRuleTest is the rule that blocks Claude from reading Trace metadata
const metadataDenyRuleTest = "Read(./.trace/metadata/**)"
...
func TestUninstallHooks_PreservesUserHooks(t *testing.T) {
  ...
  "hooks": {
    "Stop": [
      ...
      {
        "matcher": "",
        "hooks": [{"type": "command", "command": "trace hooks claude-code stop"}]
      }
    ]
  }
}`)
...
func TestInstallHooks_PreservesUserHooksOnSameType(t *testing.T) {
  ...
  "hooks": {
	...
	t.Run("PostToolUse", func(t *testing.T) {
		...
		assertHookExists(t, matchers, "Task", agentpkg.WrapProductionSilentHookCommand("trace hooks claude-code post-task"), "Trace Task hook")
		assertHookExists(t, matchers, "TodoWrite", agentpkg.WrapProductionSilentHookCommand("trace hooks claude-code post-todo"), "Trace TodoWrite hook")
	})
}
