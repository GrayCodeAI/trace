...
import (
	...
	"github.com/GrayCodeAI/trace/cmd/trace/cli/search"
...
powered by the Trace search service.
...
branch:<name>, repo:<owner/name>, and repo:* to search all accessible repos.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if query == "" && !hasFilters && (jsonOutput || !isTerminal || IsAccessibleMode()) {
				return errors.New("query required when using --json, accessible mode, or piped output. Usage: trace search <query>")
			}
			...
			serviceURL := os.Getenv("TRACE_SEARCH_URL")
			if serviceURL == "" {
