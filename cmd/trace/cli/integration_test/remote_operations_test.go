...
// the trace/checkpoints/v1 branch to a bare remote after condensation.
func TestPrePush_PushesCheckpointBranchToOrigin(t *testing.T) {
	...
	if !env.BranchExists(paths.MetadataBranchName) {
		t.Fatal("trace/checkpoints/v1 should exist locally after condensation")
	}
...
func TestPrePush_NoOpWhenNoCheckpoints(t *testing.T) {
	...
	if env.BranchExistsOnRemote(bareDir, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should NOT exist on remote when no checkpoints were created")
	}
}
...
func TestPrePush_PushDisabledSkipsCheckpoints(t *testing.T) {
	...
	if env.BranchExistsOnRemote(bareDir, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should NOT be on remote when push_sessions is false")
	}
}
...
func TestPrePush_CheckpointRemoteRoutesToSeparateRemote(t *testing.T) {
	...
	if !env.BranchExistsOnRemote(bareCheckpoint, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should exist on checkpoint remote")
	}
	if env.BranchExistsOnRemote(bareOrigin, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should NOT be on origin when routed to checkpoint remote")
	}
...
func TestPrePush_CheckpointURLDerivationFailureFallsBackToOrigin(t *testing.T) {
	...
	if !env.BranchExistsOnRemote(bareDir, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should be on origin when checkpoint_remote is unavailable (fork/fallback)")
	}
}
...
func TestCloneAndResume_FetchesCheckpointMetadata(t *testing.T) {
	...
	if !cloneEnv.BranchExists(paths.MetadataBranchName) {
		t.Fatal("trace/checkpoints/v1 should exist in clone after fetch")
	}
...
func TestGracefulDegradation_UnreachableCheckpointRemotePushContinues(t *testing.T) {
	...
	if !env.BranchExistsOnRemote(bareOrigin, paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should be on origin when checkpoint_remote URL derivation fails")
	}
}
...
func TestGracefulDegradation_UnreachableCheckpointRemoteOnCloneIsSilent(t *testing.T) {
	...
	if !cloneEnv.BranchExists(paths.MetadataBranchName) {
		t.Error("trace/checkpoints/v1 should exist locally after session + commit in clone")
	}
}
...
// `trace resume` fetches the primary repository branch (the user's feature
...
// feature branch locally. With filtered_fetches enabled, `trace resume`
