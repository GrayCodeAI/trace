...
func TestExplain_CommitWithoutCheckpoint(t *testing.T) {
	t.Parallel()
	env := NewFeatureBranchEnv(t)
	// Create a regular commit without Trace-Checkpoint trailer
	env.WriteFile("test.txt", "content")
	env.GitAdd("test.txt")
	env.GitCommit("Regular commit without Trace trailer")
	...
	// Should show "No associated Trace checkpoint" failure block
...
func TestExplain_CommitWithCheckpointTrailer(t *testing.T) {
	t.Parallel()
	env := NewFeatureBranchEnv(t)
	// Create a commit with Trace-Checkpoint trailer
	env.WriteFile("test.txt", "content")
	env.GitAdd("test.txt")
...
// with v2 disabled and enabled, verifying that `trace explain` (branch listing)
...
func TestExplain_BranchListingShowsCheckpointsAndPrompts(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// `trace explain` (no flags) should show the branch listing with the checkpoint.
			output, err := env.RunCLIWithError("checkpoint", "explain")
...
// trace/checkpoints/v1 branch doesn't exist locally (e.g., reviewing
// someone else's PR).
...
// locally-ahead trace/checkpoints/v1 branch. If the fetch path force-updates
...
func TestExplain_CheckpointFetchDoesNotRewindLocalAheadBranch(t *testing.T) {
	...
	// Checkpoint B: commit locally, DO NOT push. Local trace/checkpoints/v1 is
...
// locally-ahead v2 ref (refs/trace/v2/main). v2 uses a direct-write refspec
// (`+refs/trace/v2/main:refs/trace/v2/main`), so a naive fetch force-rewinds
...
func TestExplain_BranchListingV2OnlyAfterV1Deleted(t *testing.T) {
	...
	repo, err := git.PlainOpen(env.RepoDir)
	require.NoError(t, err)
	_ = repo.Storer.RemoveReference(plumbing.NewBranchReferenceName("trace/checkpoints/v1"))
...
// `trace explain --checkpoint <id>` used to fail with "checkpoint not
