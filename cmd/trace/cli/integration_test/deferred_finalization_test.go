...
//
// This verifies that the final transcript on trace/checkpoints/v1 includes
...
func TestShadow_DeferredTranscriptFinalization(t *testing.T) {
	...
	// Using custom commit with verbose output for debugging
	{
		...
		prepCmd.Env = append(testutil.GitIsolatedEnv(), "TRACE_TEST_TTY=1")
	...
	if !env.BranchExists(paths.MetadataBranchName) {
		t.Fatal("trace/checkpoints/v1 branch should exist")
	}
...
func TestShadow_CarryForward_ActiveSession(t *testing.T) {
	...
	t.Logf("Trace branches after first commit: %v", branches)
	...
	// Only trace/checkpoints/v1 should exist. Extra shadow branches (trace/<hash>-<hash>)
	// indicate a regression in carry-forward cleanup.
	branchesAfterAll := env.ListBranchesWithPrefix("trace/")
	...
	t.Logf("Trace branches after all commits: %v", branchesAfterAll)
...
// trace/checkpoints/v1 — leaving a "phantom" trailer pointing to nothing.
func TestShadow_AgentCommitsMidTurn_UserCommitsRemainder(t *testing.T) {
	...
	branchesBefore := env.ListBranchesWithPrefix("trace/")
	...
	// CRITICAL: The user's checkpoint must exist on trace/checkpoints/v1.
