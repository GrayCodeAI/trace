...
// RunEnableWithAccessibleMode runs `trace enable` without --strategy flag in accessible mode.
...
// SetEnabled updates the enabled state in .trace/settings file
...
func TestRewindBlockedWhenDisabled(t *testing.T) {
	...
	if !strings.Contains(stdout, "Trace is disabled") {
		t.Errorf("Expected disabled message, got: %s", stdout)
	}
	if !strings.Contains(stdout, "trace enable") {
		t.Errorf("Expected message to mention 'trace enable', got: %s", stdout)
	}
}
...
func TestHooksSilentWhenDisabled(t *testing.T) {
	...
	statePath := filepath.Join(env.RepoDir, ".trace", "tmp", "pre-prompt-test-session-disabled.json")
...
func TestEnableDefaultStrategy(t *testing.T) {
	...
	// Create a basic test environment with just a git repo (no Trace init)
	env := NewTestEnv(t)
	env.InitRepo()

	// Run trace enable without --strategy flag
