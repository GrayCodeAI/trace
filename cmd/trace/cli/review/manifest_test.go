...
import (
	...
	agenttypes "github.com/GrayCodeAI/trace/cmd/trace/cli/agent/types"
...
func TestWriteReviewCompletionFooter_PrintsExactFixCommands(t *testing.T) {
	...
	for _, want := range []string{
		"Review complete.",
		"trace review --fix claude-session --all",
		"trace review --fix claude-session",
	} {
...
func TestPrintReviewFindingsList_PrintsProductionCommandName(t *testing.T) {
	...
	os.Args = []string{"/tmp/local-build/trace"}
	...
	got := b.String()
	if strings.Contains(got, "/tmp/local-build/trace") {
		t.Fatalf("findings list should not print local binary path:\n%s", got)
	}
	if !strings.Contains(got, "trace review --fix claude-session --all") {
...
func TestWarnManifestNotWritten_PrintsReasonAndDiagnosticHints(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`trace review --findings` and `trace review --fix` will not see this run.",
