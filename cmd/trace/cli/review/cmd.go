...
//
// cmd.go provides NewCommand(), the cobra entry point for `trace review`.
...
import (
	...
	"github.com/GrayCodeAI/trace/cmd/trace/cli/interactive"
	"github.com/GrayCodeAI/trace/cmd/trace/cli/logging"
	"github.com/GrayCodeAI/trace/cmd/trace/cli/paths"
	reviewtypes "github.com/GrayCodeAI/trace/cmd/trace/cli/review/types"
...
// provided by buildReviewDeps in cmd/trace/cli/review_bridge.go and
...
type Deps struct {
	...
	// Production wiring resolves the same provider trace explain uses.
...
// NewCommand returns the `trace review` cobra command wired with the
...
func NewCommand(deps Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// users who know about it can still run `trace review` / trace
		// review --help` and the command works normally.
		Hidden: true,
		Short:  "Run configured review skills against the current branch",
		Long: `Run the review skills configured in .trace/settings.json against
...
func runReview(ctx context.Context, cmd *cobra.Command, agentOverride string, deps Deps, innerDeps runReviewDeps) error {
	...
	// settings with an empty TraceSettings{}.
	s, err := settings.Load(ctx)
	if err != nil {
		cmd.SilenceUsage = true
		fmt.Fprintf(cmd.ErrOrStderr(), "Failed to load settings: %v\n", err)
		fmt.Fprintln(cmd.ErrOrStderr(), "Fix `.trace/settings.json` and re-run `trace review`.")
...
func runSingleAgentPath(
	...
	if !found {
		...
		fmt.Fprintf(cmd.ErrOrStderr(),
			"Hooks are not installed for %q. Run `trace configure --agent %s` first, "+
...
func runMultiAgentPath(
	...
	//     forever — happens when trace review is invoked from inside an
...
// agent host like Claude Code invoking `trace review`) must NOT use the
...
// review skills ran but findings were not persisted, so `trace review
// --findings` and `trace review --fix` will not see this run. The reason
...
func warnManifestNotWritten(out io.Writer, reason string) {
	...
	fmt.Fprintln(out, "  `trace review --findings` and `trace review --fix` will not see this run.")
	fmt.Fprintln(out, "  Re-run with `TRACE_LOG_LEVEL=debug` for diagnostic detail.")
}
