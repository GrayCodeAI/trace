...
// every `trace review --help` invocation in tests). Note: side effects are
...
// to .trace/settings.local.json (via persistSummaryProviderSelection) on
...
// prompt, not by every `trace review --help`.
//
...
// are not silenced — they live in .trace/logs/.
//
// Note: first call against an unconfigured repo will write
// .trace/settings.local.json — see the lazySynthesisProvider doc above.
...
// SummaryProvider through logging so it ends up in .trace/logs/ rather than
