...
func runAuthList(ctx context.Context, w io.Writer, store tokenStore, list authTokenLister, baseURL string, jsonOut bool) error {
	...
	if token == "" {
		return fmt.Errorf("not logged in to %s; run 'trace login' first", baseURL)
	}
...
// authListStyles holds the lipgloss styles for `trace auth list`. Mirrors the
...
func newAuthRevokeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Revoke a specific API token. Use --current to revoke the token used by this CLI (equivalent to 'trace logout').",
