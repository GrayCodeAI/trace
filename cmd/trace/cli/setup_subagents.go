...
func searchSubagentTemplate(agentName types.AgentName) (string, []byte, bool) {
	...
	case agent.AgentNameClaudeCode:
		return filepath.Join(".claude", "agents", "trace-search.md"), []byte(strings.TrimSpace(claudeSearchSubagentTemplate) + "\n"), true
	case agent.AgentNameCodex:
		return filepath.Join(".codex", "agents", "trace-search.toml"), []byte(strings.TrimSpace(codexSearchSubagentTemplate) + "\n"), true
	case agent.AgentNameGemini:
		return filepath.Join(".gemini", "agents", "trace-search.md"), []byte(strings.TrimSpace(geminiSearchSubagentTemplate) + "\n"), true
	default:
		return "", nil, false
	}
}
...
<!-- ` + traceManagedSearchSubagentMarker + ` -->
