...
import (
	...
	dispatchpkg "github.com/GrayCodeAI/trace/cmd/trace/cli/dispatch"
...
func TestParseDispatchFlags_ServerReposAreAllowed(t *testing.T) {
	...
	opts, err := parseDispatchFlags(
		...
		"",
		false,
		[]string{"GrayCodeAI/cli", "GrayCodeAI/trace.io"},
		"",
		false,
	)
	if err != nil {
...
func TestParseDispatchFlags_NormalizesRepoScopeValues(t *testing.T) {
	...
	opts, err := parseDispatchFlags(
		...
		"",
		false,
		[]string{" GrayCodeAI/cli ", "", "GrayCodeAI/cli", " otherco/service ", "   "},
		"",
		false,
	)
	if err != nil {
	...
	if got := strings.Join(opts.RepoPaths, ","); got != "GrayCodeAI/cli,otherco/service" {
...
func TestNewDispatchCmd_NonTerminalPrintsPlainMarkdown(t *testing.T) {
	...
	cmd.SetOut(&stdout)
	cmd.SetErr(&stderr)
	cmd.SetArgs([]string{"--repos", "GrayCodeAI/cli"})
