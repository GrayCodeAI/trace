...
// Store provides CRUD operations for trail metadata on the trace/trails/v1 branch.
type Store struct {
	repo *git.Repository
}
...
// EnsureBranch creates the trace/trails/v1 orphan branch if it doesn't exist.
...
// Write writes trail metadata, discussion, and checkpoints to the trace/trails/v1 branch.
...
// Read reads a trail by its ID from the trace/trails/v1 branch.
...
// List returns all trail metadata from the trace/trails/v1 branch.
...
// Delete removes a trail from the trace/trails/v1 branch.
...
// getBranchRef returns the commit hash and root tree hash for the trace/trails/v1 branch HEAD
...
// getBranchTree returns the tree for the trace/trails/v1 branch HEAD.
