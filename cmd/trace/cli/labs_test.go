...
func TestLabsCmd_HelpShowsExperimentalCommandList(t *testing.T) {
	...
	got := out.String()
	for _, want := range []string{"Labs", "trace review"} {
		if !strings.Contains(got, want) {
...
func TestLabsCmd_RejectsTopicWithoutRunningIt(t *testing.T) {
	...
	if !strings.Contains(errOut.String(), "trace review --help") {
...
func TestRootHelp_ShowsLabsButHidesReview(t *testing.T) {
	...
	got := out.String()
	if !strings.Contains(got, "labs") || !strings.Contains(got, "Explore experimental Trace workflows") {
