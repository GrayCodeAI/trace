...
import (
	...
	"github.com/GrayCodeAI/trace/cmd/trace/cli/summarize"
...
func TestResolveCheckpointSummaryProvider_UsesConfiguredProvider(t *testing.T) {
	...
	loadSummarySettings = func(context.Context) (*settings.TraceSettings, error) {
...
func TestResolveCheckpointSummaryProvider_SavesSingleInstalledProvider(t *testing.T) {
	...
	loadSummarySettings = func(context.Context) (*settings.TraceSettings, error) {
		return &settings.TraceSettings{Enabled: true}, nil
	}
	...
	localPath := filepath.Join(tmpDir, ".trace", "settings.local.json")
	...
	projectPath := filepath.Join(tmpDir, ".trace", "settings.json")
...
func TestResolveCheckpointSummaryProvider_ConfiguredExternalProvider(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, ".trace", "settings.json"), []byte(`{"enabled":true,"external_agents":true,"summary_generation":{"provider":"`+providerName+`","model":"external-model"}}`), 0o644); err != nil {
...
func TestPersistSummaryProviderSelection_ExternalFlipsFlagAndReturnsSignal(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, ".trace", "settings.json"), []byte(`{"enabled":true}`), 0o644); err != nil {
		t.Fatalf("write settings: %v", err)
	}
	...
	s, err := settings.LoadFromFile(filepath.Join(tmpDir, ".trace", "settings.local.json"))
	if err != nil {
		t.Fatalf("LoadFromFile() error = %v", err)
	}
	if !s.ExternalAgents {
...
func TestPersistSummaryProviderSelection_ExternalAlreadyEnabledNoSignal(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, ".trace", "settings.local.json"), []byte(`{"external_agents":true}`), 0o644); err != nil {
