...
func TestInstallGitHook_LocalDevCommandPrefix(t *testing.T) {
	...
	for _, hook := range gitHookNames {
		...
		content := string(data)
		if !strings.Contains(content, "go run ./cmd/trace/main.go") {
			t.Errorf("hook %s should use 'go run' prefix when localDev=true, got:\n%s", hook, content)
		}
		if strings.Contains(content, "\ntrace ") {
			t.Errorf("hook %s should not use bare 'trace' prefix when localDev=true", hook)
		}
	}
	...
	for _, hook := range gitHookNames {
		...
		if !strings.Contains(content, "\ntrace ") {
			t.Errorf("hook %s should use bare 'trace' prefix when localDev=false", hook)
		}
	}
}
...
func TestInstallGitHook_AbsoluteGitHookPath(t *testing.T) {
	...
	for _, hook := range gitHookNames {
		...
		if strings.Contains(content, "\ntrace ") {
			t.Errorf("hook %s should not use bare 'trace' prefix when absolutePath=true", hook)
		}
	}
}
