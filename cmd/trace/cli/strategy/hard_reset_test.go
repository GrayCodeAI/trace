...
// does not delete untracked directories like .trace/ and .worktrees/.
//
...
func TestHardResetWithProtection_PreservesProtectedDirs(t *testing.T) {
	...
	if err := os.WriteFile(gitignore, []byte(".trace/\n.worktrees/\n"), 0o644); err != nil {
		t.Fatalf("failed to create .gitignore: %v", err)
	}

	// Create protected directories with content (these are untracked/ignored)
	traceDir := filepath.Join(repoDir, ".trace")
	if err := os.MkdirAll(filepath.Join(traceDir, "metadata"), 0o755); err != nil {
		t.Fatalf("failed to create .trace dir: %v", err)
	}
	traceContent := "important session metadata"
	if err := os.WriteFile(filepath.Join(traceDir, "metadata", "session.json"), []byte(traceContent), 0o644); err != nil {
	...
	if _, err := os.Stat(traceDir); os.IsNotExist(err) {
		t.Error(".trace/ directory was deleted by HardResetWithProtection - THIS IS A REGRESSION")
	}
	content, err := os.ReadFile(filepath.Join(traceDir, "metadata", "session.json"))
	if err != nil {
		t.Errorf("failed to read .trace file after reset: %v - directory may have been deleted", err)
	} else if string(content) != traceContent {
		t.Errorf(".trace content changed: got %q, want %q", content, traceContent)
	}
