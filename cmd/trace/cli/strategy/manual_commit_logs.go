...
// For manual-commit strategy, returns the sharded path on trace/checkpoints/v1 branch.
...
// For manual-commit strategy, metadata lives on the trace/checkpoints/v1 branch.
func (s *ManualCommitStrategy) GetSessionMetadataRef(ctx context.Context, _ string) string {
	...
	// The tip of trace/checkpoints/v1 contains all condensed sessions
...
// For manual-commit strategy, metadata is stored at sharded paths on trace/checkpoints/v1 branch.
...
// Returns active sessions from .git/trace-sessions/ that haven't yet been condensed.
...
// sessionID is expected to be an Trace session ID (already date-prefixed like "2026-01-12-abc123").
