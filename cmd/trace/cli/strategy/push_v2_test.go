...
func TestShortRefName(t *testing.T) {
	...
	tests := []struct {
		...
		{"refs/trace/checkpoints/v2/main", "v2/main"},
		{"refs/trace/checkpoints/v2/full/current", "v2/full/current"},
		{"refs/trace/checkpoints/v2/full/0000000000001", "v2/full/0000000000001"},
...
func TestPushV2Refs_PushesAllRefs(t *testing.T) {
	...
	assert.Contains(t, output, "[trace] Syncing and pushing v2 checkpoints...")
	assert.Contains(t, output, "[trace] Pushing v2/main, v2/full/current, v2/full/0000000000002...")
	assert.Contains(t, output, "[trace] All v2 checkpoints pushed")
	assert.NotContains(t, output, "[trace] Successfully pushed", "successful refs should only be listed on partial failure")
...
func TestPrintV2PartialPushResult(t *testing.T) {
	...
	assert.Contains(t, output.String(), "[trace] Successfully pushed v2/main")
	assert.Contains(t, output.String(), "[trace] Warning: couldn't sync v2/full/current: fetch failed")
	assert.NotContains(t, output.String(), "[trace] All v2 checkpoints pushed")
}
...
func TestParsePushRefResults_MultiRefPorcelain(t *testing.T) {
	...
	output := strings.Join([]string{
		...
		"*\trefs/trace/checkpoints/v2/main:refs/trace/checkpoints/v2/main\t[new reference]",
		"!\trefs/trace/checkpoints/v2/full/current:refs/trace/checkpoints/v2/full/current\t[rejected] (non-fast-forward)",
		"=\trefs/trace/checkpoints/v2/full/0000000000002:refs/trace/checkpoints/v2/full/0000000000002\tup to date",
	}, "\n")
...
func TestParsePushRefResults_GenericRejectionDoesNotBecomeNonFastForward(t *testing.T) {
	...
	output := "!\trefs/trace/checkpoints/v2/main:refs/trace/checkpoints/v2/main\t[remote rejected] auth rejected by server"
...
func TestPushV2Refs_UnreachableTarget_NamesFailedRef(t *testing.T) {
	...
	assert.Contains(t, output, "[trace] Syncing and pushing v2 checkpoints...")
	assert.Contains(t, output, "[trace] Pushing v2/main...")
	assert.Contains(t, output, "[trace] Warning: failed to push v2/main:")
	assert.NotContains(t, output, "[trace] Warning: couldn't sync v2/main:")
	assert.NotContains(t, output, "[trace] All v2 checkpoints pushed")
