...
func newSessionsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:   "Manage agent sessions tracked by Trace",
...
Examples:
  ...
  trace session info <session-id>         Show session details
  trace session info <session-id> --json  Output as JSON
...
Examples:
  trace sessions stop                     No sessions: exits. One session: confirm and stop. Multiple: show selector
  trace sessions stop <session-id>        Stop a specific session by ID
  trace sessions stop --all               Stop all active sessions
  trace sessions stop --force             Skip confirmation prompt`,
...
func runStop(ctx context.Context, cmd *cobra.Command, sessionID string, all, force bool) error {
	...
	// This aligns with `trace status` which displays sessions globally.
...
// any session visible in `trace status` should also be visible in `sessions stop`.
...
func newListCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `List all sessions tracked by Trace, including ended sessions.
...
Examples:
  trace sessions info <session-id>
  trace sessions info <session-id> --json`,
