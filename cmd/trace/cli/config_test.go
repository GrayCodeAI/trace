...
func TestLoadTraceSettings_EnabledDefaultsToTrue(t *testing.T) {
	...
	settings, err := LoadTraceSettings(context.Background())
	if err != nil {
		t.Fatalf("LoadTraceSettings(context.Background()) error = %v", err)
	}
	if !settings.Enabled {
	...
	settingsContent := `{}`
	if err := os.WriteFile(TraceSettingsFile, []byte(settingsContent), 0o644); err != nil {
...
func TestSaveTraceSettings_PreservesEnabled(t *testing.T) {
	tmpDir := t.TempDir()
	t.Chdir(tmpDir)
	...
	if err := SaveTraceSettings(context.Background(), settings); err != nil {
		t.Fatalf("SaveTraceSettings() error = %v", err)
	}

	// Load and verify
	loaded, err := LoadTraceSettings(context.Background())
	if err != nil {
		t.Fatalf("LoadTraceSettings(context.Background()) error = %v", err)
	}
	if loaded.Enabled {
...
// setupLocalOverrideTestDir creates a temp directory with .trace folder for testing
...
func TestLoadTraceSettings_LocalOverridesStrategy(t *testing.T) {
	setupLocalOverrideTestDir(t)

	baseSettings := testSettingsEnabled
	if err := os.WriteFile(TraceSettingsFile, []byte(baseSettings), 0o644); err != nil {
		t.Fatalf("Failed to write settings file: %v", err)
	}

	localSettings := testSettingsEnabled
	if err := os.WriteFile(TraceSettingsLocalFile, []byte(localSettings), 0o644); err != nil {
...
func TestLoadTraceSettings_LocalOverridesEnabled(t *testing.T) {
	setupLocalOverrideTestDir(t)
...
func TestLoadTraceSettings_LocalOverridesLocalDev(t *testing.T) {
	setupLocalOverrideTestDir(t)
...
func TestLoadTraceSettings_LocalMergesStrategyOptions(t *testing.T) {
	setupLocalOverrideTestDir(t)
...
func TestLoadTraceSettings_OnlyLocalFileExists(t *testing.T) {
	setupLocalOverrideTestDir(t)
...
func TestLoadTraceSettings_RejectsUnknownKeysInBase(t *testing.T) {
	setupLocalOverrideTestDir(t)
	...
	_, err := LoadTraceSettings(context.Background())
	if err == nil {
		t.Fatal("LoadTraceSettings(context.Background()) should return error for unknown key")
	}
...
func TestLoadTraceSettings_RejectsUnknownKeysInLocal(t *testing.T) {
	setupLocalOverrideTestDir(t)
	...
	_, err := LoadTraceSettings(context.Background())
	if err == nil {
		t.Fatal("LoadTraceSettings(context.Background()) should return error for unknown key in local settings")
	}
	if !strings.Contains(err.Error(), "unknown field") {
		t.Errorf("Error should mention 'unknown field', got: %v", err)
	}
