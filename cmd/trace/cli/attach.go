...
func runAttach(ctx context.Context, w io.Writer, sessionID string, agentName types.AgentName, force bool) error {
	...
	// Write directly to trace/checkpoints/v1.
...
// metadata fetch fallback chain used by `trace resume` (which advances the
...
func ensureCheckpointAvailable(ctx, logCtx context.Context, repo *git.Repository, checkpointID id.CheckpointID, isExistingCheckpoint bool) (*git.Repository, error) {
	...
	// chain `trace resume` uses for the active storage version (v2 refs live
	// under refs/trace/, not refs/heads/, so v1 and v2 need different
	// refspecs).
	...
	branchDescription := "trace/checkpoints/v1 branch"
	if v2Only {
...
// trace/checkpoints/v1 branch (remote-tracking alone is not enough — see
...
// paste to pull the missing metadata ref. v2 refs live under refs/trace/
...
func suggestCheckpointFetchCommand(ctx context.Context, v2Only bool) string {
	ref := "trace/checkpoints/v1:trace/checkpoints/v1"
	if v2Only {
...
// If HEAD already has an Trace-Checkpoint trailer, reuses that ID (the session
