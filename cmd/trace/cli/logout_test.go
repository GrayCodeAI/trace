...
func TestRunLogout_RevokesServerSideThenDeletesLocally(t *testing.T) {
	...
	store.tokens["https://trace.io"] = testLogoutToken
	...
	err := runLogout(context.Background(), &out, &errOut, store, revoke, "https://trace.io")
	if err != nil {
	...
	if !store.deleted["https://trace.io"] {
		t.Fatal("expected token to be deleted for https://trace.io")
	}
