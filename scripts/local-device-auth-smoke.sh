...
: "${TRACE_API_BASE_URL:=http://localhost:8787}"

LOG_FILE="$(mktemp -t trace-device-auth-smoke.XXXXXX.log)"
cleanup() {
...
echo "Starting device auth login against ${TRACE_API_BASE_URL}"
TRACE_TEST_TTY=0 TRACE_API_BASE_URL="${TRACE_API_BASE_URL}" go run ./cmd/trace login --insecure-http-auth >"${LOG_FILE}" 2>&1 &
LOGIN_PID=$!
...
AUTH_FILE="${HOME}/.config/trace/auth.json"

python3 - <<'PY' "${AUTH_FILE}" "${TRACE_API_BASE_URL}"
import json
import pathlib
import sys
