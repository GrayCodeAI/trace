...
main() {
    ...
    local archive_name="trace_${os}_${arch}.tar.gz"
    ...
    local binary_path="${tmp_dir}/trace"

    chmod +x "$binary_path"

    info "Installing to ${install_dir}..."
    local install_path="${install_dir}/trace"
    ...
    if "$install_path" version &> /dev/null; then
        success "Trace CLI installed to ${install_path}"
    else
    ...
    local path_binary
    path_binary=$(command -v "trace" 2>/dev/null || true)
    if [[ -n "$path_binary" && ! "$path_binary" -ef "$install_path" ]]; then
        # This case is a bit weird, because some other 'trace' is found on PATH.  Warn user.
        echo ""
        echo -e "${YELLOW}!${NC} ${BOLD}WARNING: PATH conflict detected${NC}"
        echo -e "${YELLOW}!${NC}"
        echo -e "${YELLOW}!${NC} Installed to: ${install_path}"
        echo -e "${YELLOW}!${NC} But 'trace' resolves to: ${path_binary}"
        echo -e "${YELLOW}!${NC}"
        echo -e "${YELLOW}!${NC} Your PATH may have another version of Trace CLI. To fix:"
        ...
        echo ""
        echo -e "  Add ${BOLD}trace${NC} to your PATH:"
        echo ""
        if [[ "$shell_name" == "fish" ]]; then
            ...
            echo ""
            echo -e "  Restart your terminal, then run ${BOLD}trace${NC} to get started."
            exit 0
        fi
        echo ""
        echo -e "  Restart your terminal, then run ${BOLD}trace${NC} to get started."
