...
echo -e "${BLUE}Building trace CLI from: $CLI_DIR${NC}"
...
TRACE_BIN="$TRACE_BIN_DIR/trace"
if ! go build -o "$TRACE_BIN" "$CLI_DIR/cmd/trace"; then
    echo -e "${RED}Failed to build trace CLI${NC}"
    exit 1
fi
chmod +x "$TRACE_BIN"
echo -e "${GREEN}Built: $TRACE_BIN${NC}"

# Add the binary directory to PATH so Claude's hooks find it
export PATH="$TRACE_BIN_DIR:$PATH"
echo -e "${GREEN}Added to PATH: $TRACE_BIN_DIR${NC}"

# Verify the right binary is being used
echo -e "${BLUE}Verifying trace location:${NC} $(which trace)"
...
# Enable trace
echo -e "${BLUE}=== Step 3: Enable trace ===${NC}"
trace enable --strategy manual-commit

# Commit the setup files to establish a clean baseline
echo -e "${BLUE}=== Step 3b: Commit setup files (clean baseline) ===${NC}"
git add .claude/ .trace/
git commit -m "Setup trace tracking"
echo -e "${GREEN}Baseline established - .claude/ and .trace/ are now committed${NC}"
...
# Check for Trace-Checkpoint trailer
echo ""
echo -e "${BLUE}=== Step 11: Check attribution in commit ===${NC}"
CHECKPOINT_ID=$(git log -1 --format=%B | grep "Trace-Checkpoint:" | cut -d: -f2 | tr -d ' ')
if [[ -n "$CHECKPOINT_ID" ]]; then
    echo -e "${GREEN}Found Trace-Checkpoint: $CHECKPOINT_ID${NC}"
    ...
    echo ""
    echo -e "${BLUE}=== Step 12: Inspect metadata on trace/checkpoints/v1 branch ===${NC}"
    echo "Looking for metadata at: $METADATA_PATH"

    # Read metadata.json from trace/checkpoints/v1 branch
    if git show "trace/checkpoints/v1:${METADATA_PATH}" > /dev/null 2>&1; then
        echo -e "${GREEN}Found metadata.json:${NC}"
        git show "trace/checkpoints/v1:${METADATA_PATH}" | jq .

        # Extract and display attribution specifically
        echo ""
        echo -e "${BLUE}=== Step 13: Attribution Analysis ===${NC}"
        ATTRIBUTION=$(git show "trace/checkpoints/v1:${METADATA_PATH}" | jq -r '.initial_attribution // empty')
        if [[ -n "$ATTRIBUTION" && "$ATTRIBUTION" != "null" ]]; then
            ...
            if [[ -d ".trace/logs" ]]; then
                grep -i "attribution" .trace/logs/*.log 2>/dev/null | tail -30 || echo "(no attribution logs found)"
            else
                echo "(no .trace/logs directory)"
            fi
        fi
        ...
        git show "trace/checkpoints/v1:${METADATA_PATH}" | jq -r '.files_touched[]?' 2>/dev/null || echo "(none)"
        ...
        git ls-tree -r --name-only "trace/checkpoints/v1" | grep "^${SHARD_PREFIX}/${SHARD_SUFFIX}/" | head -20

    else
        echo -e "${RED}Could not find metadata at $METADATA_PATH${NC}"
        echo "Checking what's on trace/checkpoints/v1 branch:"
        git ls-tree -r --name-only "trace/checkpoints/v1" 2>/dev/null | head -20 || echo "(branch may not exist)"
    fi
else
    echo -e "${YELLOW}No Trace-Checkpoint trailer found (user may have removed it)${NC}"
fi

# Show rewind points summary
echo ""
echo -e "${BLUE}=== Step 15: Rewind points summary ===${NC}"
trace rewind --list | jq -r '.[] | "  \(.id[0:8])... - \(.message[0:60])"' 2>/dev/null || echo "  (no rewind points)"
...
echo "  6. Metadata inspection on trace/checkpoints/v1 branch"
echo ""
...
if [[ "$KEEP_REPO" == "true" ]]; then
    ...
    echo "  cd $TEST_DIR"
    echo "  git log trace/checkpoints/v1 --oneline"
    echo "  git show trace/checkpoints/v1:<checkpoint-path>/metadata.json | jq ."
fi
