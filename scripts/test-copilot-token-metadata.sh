...
Trace hooks, and prints the resulting raw checkpoint metadata plus session state.
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/trace-copilot-token-metadata.XXXXXX")"
...
(cd "$ROOT_DIR" && GOCACHE="$TMP_DIR/go-build" go build -o "$BIN_DIR/trace" ./cmd/trace)

export PATH="$BIN_DIR:$PATH"
export COPILOT_PROJECT_DIR="$ROOT_DIR"
export TRACE_TEST_COPILOT_SESSION_DIR="$SESSION_ROOT"
export TRACE_TEST_TTY=1

run_hook() {
    local hook_name="$1"
    local payload="$2"

    printf '%s' "$payload" | trace hooks copilot-cli "$hook_name" >/dev/null
}
...
checkpoint_metadata_json() {
    ...
    git show "trace/checkpoints/v1:${checkpoint_id:0:2}/${checkpoint_id:2}/0/metadata.json"
}
...
trace enable --agent copilot-cli >/dev/null
...
STATE_FILE="$REPO_DIR/.git/trace-sessions/$SESSION_ID.json"
