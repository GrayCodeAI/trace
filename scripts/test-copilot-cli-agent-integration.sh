...
# Tests hook mechanism and captures payloads for the Trace CLI integration.

AGENT_NAME="Copilot CLI"
AGENT_SLUG="copilot-cli"
AGENT_BIN="copilot"
PROBE_DIR=".trace/tmp/probe-${AGENT_SLUG}-$(date +%s)"
HOOK_FILE=".github/hooks/trace-probe.json"
KEEP_CONFIG=false
MANUAL_LIVE=false
RUN_CMD=""
    ...
    "sessionStart": [
      {
        ...
        "comment": "Trace CLI integration probe"
      }
    ],
    "sessionEnd": [
      {
...
check_event() {
    ...
    if $found; then
        pass "$event_name → $trace_event"
    else
        warn "$event_name → $trace_event (not captured)"
    fi
}
