...
```bash
go build -o /tmp/trace-bin ./cmd/trace
```
...
| Check | Result |
|-------|--------|
| Active branch | Optional Trace-Checkpoint: trailer |
| Session state | ✓ Exists |
| Shadow branch | ✓ trace/{hash} |
| Metadata branch | ✓ trace/checkpoints/v1 |
...
- Active branch commits: **NO modifications** (no commits created by Trace)
- Shadow branches: `trace/<commit-hash[:7]>` created for checkpoints
- Metadata: stored on both shadow branches and `trace/checkpoints/v1` branch (condensed on user commits)
...
echo "{\"session_id\": \"$SESSION_ID\", \"transcript_path\": \"$TRANSCRIPT_DIR/transcript.jsonl\", \"tool_use_id\": \"$TOOL_USE_ID\", \"tool_input\": {\"subagent_type\": \"dev\", \"description\": \"Test task\"}}" | \
  TRACE_TEST_CLAUDE_PROJECT_DIR="$TRANSCRIPT_DIR" \
  /tmp/trace-bin hooks claude-code pre-task
...
echo "{\"session_id\": \"$SESSION_ID\", \"transcript_path\": \"$TRANSCRIPT_DIR/transcript.jsonl\", \"tool_use_id\": \"$TOOL_USE_ID\", \"tool_response\": {\"agentId\": \"test-agent\"}}" | \
  TRACE_TEST_CLAUDE_PROJECT_DIR="$TRANSCRIPT_DIR" \
  /tmp/trace-bin hooks claude-code post-task

# Verify task checkpoint created
/tmp/trace-bin rewind --list | jq '.[] | select(.is_task_checkpoint == true)'
```
...
# Verify logs condensed to trace/checkpoints/v1
git show trace/checkpoints/v1 --stat | grep -E "^[0-9a-f]{2}/[0-9a-f]"

# Verify shadow branch still exists
git branch -a | grep "trace/[0-9a-f]"
```

## Available Claude Code Hooks

All hooks use the command: `trace hooks claude-code <hook-name>`
