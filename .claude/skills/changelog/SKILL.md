...
To determine if a contributor is external, check for an `@trace.io` email:

```bash
git log --all --format='%an <%ae>' --author="<name>" | sort -u
```

If they have an `@trace.io` email anywhere in git history, they are **internal**. Only list external contributors in the Thanks section.
...
- Feature description with context ([#123](https://github.com/GrayCodeAI/trace/pull/123))

### Changed

- Change description ([#456](https://github.com/GrayCodeAI/trace/pull/456))

### Fixed

- Bug fix description ([#789](https://github.com/GrayCodeAI/trace/pull/789))

### Housekeeping

- Maintenance item ([#101](https://github.com/GrayCodeAI/trace/pull/101))
...
- **PR links**: always use full URL format `[#N](https://github.com/GrayCodeAI/trace/pull/N)`
