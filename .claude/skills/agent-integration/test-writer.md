...
- The research command's one-pager at `cmd/trace/cli/agent/$AGENT_PACKAGE/AGENT.md`
...
1. `e2e/tests/main_test.go` — `TestMain` builds the CLI binary (via `trace.BinPath()`), runs preflight checks for required binaries (git, tmux, agent CLIs), sets up artifact directories, and configures env
2. `e2e/testutil/repo.go` — `RepoState` struct (holds agent, dir, artifact dir, head/checkpoint refs), `SetupRepo` (creates temp git repo, runs `trace enable`, patches settings), `ForEachAgent` (runs a test per registered agent with repo setup, concurrency gating, and timeout scaling)
3. `e2e/agents/agent.go` — `Agent` interface (`Name`, `Binary`, `TraceAgent`, `PromptPattern`, `TimeoutMultiplier`, `RunPrompt`, `StartSession`, `Bootstrap`, `IsTransientError`), `Register()` for agent self-registration in `init()`, `RegisterGate()` for concurrency limits, `AcquireSlot`/`ReleaseSlot` for gating
...
7. `e2e/trace/trace.go` — CLI wrapper: `BinPath()` (builds from source or uses `E2E_TRACE_BIN`), `Enable`, `Disable`, `RewindList`, `Rewind`, `RewindLogsOnly`, `Explain`, `ExplainGenerate`, `ExplainCommit`, `Resume`
8. `e2e/testutil/artifacts.go` — Automatic artifact capture via `t.Cleanup`: `CaptureArtifacts` saves git-log, git-tree, checkpoint metadata, trace logs, and tmux pane content
...
Read `cmd/trace/cli/agent/$AGENT_PACKAGE/AGENT.md` (the one-pager from the research phase) for all agent-specific information:
...
func (a *${AgentName}) TraceAgent() string        { return "${trace-agent-name}" }
...
func (a *${AgentName}) RunPrompt(ctx context.Context, dir string, prompt string, opts ...Option) (Output, error) {
	...
	cmd.Dir = dir
	cmd.Env = append(os.Environ(), "TRACE_TEST_TTY=0")
...
- Agent-specific config files that must exist before `trace enable`
...
- **CLI operations**: Use the `e2e/trace` package (`trace.Enable`, `trace.RewindList`, `trace.Rewind`, etc.) — never call the binary via raw `exec.Command`
