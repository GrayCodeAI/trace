...
A multi-phase toolkit for taking a new AI coding agent from unknown to fully integrated with the Trace CLI.
...
| `/agent-integration:implement` | Build agent via TDD | Go package under `cmd/trace/cli/agent/` |
...
- Agent interface: `cmd/trace/cli/agent/agent.go`
- Event types: `cmd/trace/cli/agent/event.go`
...
- Existing agents: Discover via `Glob("cmd/trace/cli/agent/*/")`
