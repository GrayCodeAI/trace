...
For each failure, follow the **Debugging Workflow** in `.claude/skills/e2e/debug.md` (steps 2-5: console.log -> test source -> trace.log -> deep dive). Collect:
- What the agent actually did (from console.log)
- What was expected (from test source)
- CLI-level errors or anomalies (from trace.log)
...
- Root cause in `cmd/trace/cli/` -> **real-bug** (product code is broken)
...
#### Strong `real-bug` signals (root cause must be in `cmd/trace/cli/`, not `e2e/`):

- `trace.log` contains `"level":"ERROR"` or panic/stack traces from CLI code
...
- Error references CLI code (panic in `cmd/trace/cli/`)

**Key question:** Is the bug in `cmd/trace/cli/` (product code) or in `e2e/` (test code)? Only the former is a `real-bug`.
...
**Test-bug (consistent failure, but root cause is in `e2e/` not `cmd/trace/cli/`):**
...
- <key artifact evidence: trace.log excerpt, console.log excerpt, etc.>
```
