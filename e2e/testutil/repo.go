...
const (
	...
	checkpointRefV1            = "trace/checkpoints/v1"
	checkpointRefV2Main        = "refs/trace/checkpoints/v2/main"
	checkpointRefV2FullCurrent = "refs/trace/checkpoints/v2/full/current"
	checkpointRefV2FullPrefix  = "refs/trace/checkpoints/v2/full/"
)
...
// with an initial commit, and runs `trace enable` for the given agent.
...
func SetupRepo(t *testing.T, agent agents.Agent) *RepoState {
	...
	// $HOME/.cache/trace-e2e-repos/ instead.
	root := ""
	if agent.Name() == "codex" {
		...
		root = filepath.Join(cache, "trace-e2e-repos")
	...
	if ea, ok := agent.(agents.ExternalAgent); ok && ea.IsExternalAgent() {
		...
		if err := os.MkdirAll(traceDir, 0o755); err != nil {
			t.Fatalf("create .trace for external agent: %v", err)
		}
		if err := os.WriteFile(filepath.Join(traceDir, "settings.json"),
			[]byte("{\"external_agents\": true}\n"), 0o644); err != nil {
			t.Fatalf("write external_agents setting: %v", err)
		}
	}

	trace.Enable(t, dir, agent.TraceAgent())
	...
	// Trace-Checkpoint trailer unconditionally. This is needed because
...
func setupGeminiTestHome(t *testing.T, repoDir string) {
	...
	agentFile := filepath.Join(repoDir, ".gemini", "agents", "trace-search.md")
	...
	acks[repoDir]["trace-search"] = hash
...
// panic and restarts the trace scenario with a fresh repository, up to
...
func PatchSettings(t *testing.T, dir string, extra map[string]any) {
	t.Helper()
	path := filepath.Join(dir, ".trace", "settings.json")
	data, err := os.ReadFile(path)
	if err != nil {
...
// trace/checkpoints/v1 branch since the test was set up, oldest first.
...
// CloneAndEnableTrace clones a bare remote into a fresh temp dir, configures
// a test git identity, enables Trace for the given agent, applies the active
// suite checkpoints mode, and commits the enable changes if needed.
func CloneAndEnableTrace(t *testing.T, bareDir string, agentName string) string {
	t.Helper()
	...
	trace.Enable(t, cloneDir, agentName)
	ApplySuiteCheckpointsMode(t, cloneDir)
	CommitIfDirty(t, cloneDir, "Enable trace in clone")
...
// `trace enable` is already idempotent and produces no repo changes.
...
// GetCheckpointTrailer extracts the Trace-Checkpoint trailer value from a
...
func GetCheckpointTrailer(t *testing.T, dir string, ref string) string {
	...
	return GitOutput(t, dir, "log", "-1", "--format=%(trailers:key=Trace-Checkpoint,valueonly)", ref)
}
