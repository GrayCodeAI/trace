...
// CaptureArtifacts captures git state, checkpoint metadata, trace logs,
...
func captureTraceLogs(t *testing.T, repoDir, outDir string) {
	t.Helper()
	logDir := filepath.Join(repoDir, ".trace", "logs")
	entries, err := os.ReadDir(logDir)
	if err != nil {
		return
	}
	dst := filepath.Join(outDir, "trace-logs")
