...
// ShadowBranches returns all shadow branches (trace/*) excluding trace/checkpoints/*.
func ShadowBranches(t *testing.T, dir string) []string {
	t.Helper()
	branches := GitOutput(t, dir, "for-each-ref", "--format=%(refname:short)", "refs/heads/trace/")
	var shadow []string
	for _, b := range strings.Split(branches, "\n") {
		b = strings.TrimSpace(b)
		if b == "" || strings.HasPrefix(b, "trace/checkpoints") {
			continue
		}
...
// AssertHasShadowBranches asserts that at least one shadow branch (trace/*)
// exists, excluding trace/checkpoints/*. Use this when the shadow branch is
...
// AssertHasCheckpointTrailer asserts the commit has an Trace-Checkpoint trailer,
...
func AssertHasCheckpointTrailer(t *testing.T, dir string, ref string) string {
	...
	require.NotEmpty(t, trailer, "no Trace-Checkpoint trailer on %s", ref)
...
// WaitForSessionIdle polls the session state files in .git/trace-sessions/
...
func WaitForSessionIdle(t *testing.T, dir string, timeout time.Duration) {
	t.Helper()
	stateDir := filepath.Join(dir, ".git", "trace-sessions")
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
...
// AssertNoCheckpointTrailer asserts the commit does NOT have an Trace-Checkpoint trailer.
func AssertNoCheckpointTrailer(t *testing.T, dir string, ref string) {
	t.Helper()
	trailer := GetCheckpointTrailer(t, dir, ref)
	assert.Empty(t, trailer, "expected no Trace-Checkpoint trailer on %s, got %q", ref, trailer)
}
