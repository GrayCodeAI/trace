...
End-to-end tests for the `trace` CLI against real agents (Claude Code, Gemini CLI, OpenCode, Codex, Cursor, Factory AI Droid, Copilot CLI).
...
├── trace/       # `trace` CLI wrapper (enable, rewind, etc.)
...
- Use the `trace` package for CLI interactions, not raw `exec.Command`.
...
| `E2E_TRACE_BIN` | Path to a pre-built `trace` binary | builds from source |
...
Artifacts are captured to `e2e/artifacts/` on every run (git-log, git-tree, console.log, checkpoint metadata, trace logs). Set `E2E_KEEP_REPOS=1` to preserve the temp repo — a symlink appears in the artifact dir pointing to it.
