...
// local checkpoint metadata ref for the active suite mode. `trace resume feature`
...
func TestResumeFromClonedRepo(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		// Enable trace in the cloned repo and commit the enable files.
		trace.Enable(t, cloneDir, s.Agent.TraceAgent())
		testutil.ApplySuiteCheckpointsMode(t, cloneDir)
		testutil.CommitIfDirty(t, cloneDir, "Enable trace in clone")

		// Run resume from the clone — should fetch metadata and succeed.
		out, err := trace.Resume(cloneDir, "feature")
		require.NoError(t, err, "trace resume failed in clone: %s", out)
