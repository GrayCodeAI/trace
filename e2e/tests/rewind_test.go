...
func TestRewindPreCommit(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		pointsAfterA := trace.RewindList(t, s.Dir)
		...
		pointsAfterB := trace.RewindList(t, s.Dir)
		...
		err = trace.Rewind(t, s.Dir, rewindID)
...
func TestRewindAfterCommit(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		pointsAfter := trace.RewindList(t, s.Dir)
		...
		err = trace.Rewind(t, s.Dir, oldID)
...
func TestRewindMultipleFiles(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		// Gemini "helpfully" creating the trace rainbow.
		_, err := s.RunPrompt(t, ctx,
...
// multiple Trace-Checkpoint trailers, `trace rewind --list` should show a
...
func TestRewindSquashMergeMultipleCheckpoints(t *testing.T) {
	testutil.ForEachAgent(t, 5*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		testutil.WaitForCheckpoint(t, s, 30*time.Second)
		cp1Ref := testutil.GitOutput(t, s.Dir, "rev-parse", "trace/checkpoints/v1")
		...
		points := trace.RewindList(t, s.Dir)

		var squashPoints []trace.RewindPoint
		...
		err = trace.RewindLogsOnly(t, s.Dir, squashHash)
