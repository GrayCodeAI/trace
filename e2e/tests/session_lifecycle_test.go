...
func TestSessionDepletedManualEditNoCheckpoint(t *testing.T) {
	testutil.ForEachAgent(t, 2*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		cpBranchAfterAgent := testutil.GitOutput(t, s.Dir, "rev-parse", "trace/checkpoints/v1")
		...
		cpBranchAfterManual := testutil.GitOutput(t, s.Dir, "rev-parse", "trace/checkpoints/v1")
...
// Trace-Checkpoint trailer from the commit message, no condensation happens.
