...
// user commits, then switches back to main and runs `trace resume feature`.
...
func TestResumeFromFeatureBranch(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		// Commit files from `trace enable` so main has a clean working tree
		...
		out, err := trace.Resume(s.Dir, "feature")
		require.NoError(t, err, "trace resume failed: %s", out)
...
// squash-merged to main and `trace resume` should restore only the latest
...
//     (e.g. "* Add red doc\n\nTrace-Checkpoint: aaa\n\n* Add blue doc\n\nTrace-Checkpoint: bbb")
...
func TestResumeSquashMergeMultipleCheckpoints(t *testing.T) {
	testutil.ForEachAgent(t, 5*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		out, err := trace.Resume(s.Dir, mainBranch)
		require.NoError(t, err, "github format: trace resume failed: %s", out)
		...
		out, err = trace.Resume(s.Dir, mainBranch)
		require.NoError(t, err, "git-cli format: trace resume failed: %s", out)
		assert.Contains(t, out, "older checkpoints skipped",
...
func TestResumeNoCheckpointOnBranch(t *testing.T) {
	testutil.ForEachAgent(t, 1*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		out, err := trace.Resume(s.Dir, "no-checkpoint")
		require.NoError(t, err, "resume should not error for missing checkpoints: %s", out)

		assert.Contains(t, out, "No Trace checkpoint found",
...
// commit on top. `trace resume --force` should still find and restore the
...
func TestResumeOlderCheckpointWithNewerCommits(t *testing.T) {
	testutil.ForEachAgent(t, 3*time.Minute, func(t *testing.T, s *testutil.RepoState, ctx context.Context) {
		...
		// trace.Resume, which bypasses the "older checkpoint" confirmation).
		s.Git(t, "checkout", mainBranch)
