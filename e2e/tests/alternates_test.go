...
func TestAlternates_RelativeObjectAlternate_CheckpointSync(t *testing.T) {
	...
	testutil.Git(t, shared, "push", originBare, "remoteside:refs/heads/trace/checkpoints/v1")
	...
	cmd := exec.Command(trace.BinPath(), "hooks", "git", "pre-push", "origin")
	cmd.Dir = work
	cmd.Env = upsertEnv(os.Environ(), "TRACE_TEST_TTY", "0", "GIT_TERMINAL_PROMPT", "0")
	combined, runErr := cmd.CombinedOutput()
	output := string(combined)
	t.Logf("trace pre-push output:\n%s", output)
	if runErr != nil {
	...
	got := testutil.GitOutput(t, originBare, "rev-parse", "refs/heads/trace/checkpoints/v1")
	if got == r1 {
