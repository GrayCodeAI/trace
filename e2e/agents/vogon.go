...
func (v *Vogon) RunPrompt(ctx context.Context, dir string, prompt string, opts ...Option) (Output, error) {
	...
	env := filterEnv(os.Environ(), "TRACE_TEST_TTY")
...
func (v *Vogon) StartSession(_ context.Context, dir string) (Session, error) {
	...
	s, err := NewTmuxSession(name, dir, []string{"TRACE_TEST_TTY"}, "env", "HOME="+homeDir, v.Binary())
	if err != nil {
		return nil, err
	}
...
// to prepare a session that `trace attach` can import.
