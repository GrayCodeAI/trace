...
func init() {
	...
	if _, err := exec.LookPath("trace-agent-roger-roger"); err != nil {
		return
	}
	Register(&RogerRoger{})
}
...
// via the external agent protocol (trace-agent-roger-roger binary in $PATH).
...
func (r *RogerRoger) RunPrompt(ctx context.Context, dir string, prompt string, opts ...Option) (Output, error) {
	...
	cmd.Env = filterEnv(os.Environ(), "TRACE_TEST_TTY")
...
func (r *RogerRoger) StartSession(_ context.Context, dir string) (Session, error) {
	...
	s, err := NewTmuxSession(name, dir, []string{"TRACE_TEST_TTY"}, r.Binary())
	if err != nil {
		return nil, err
	}
