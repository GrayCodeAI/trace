...
func TestCodexCommandHookHash_MatchesCanonicalShape(t *testing.T) {
	...
	const wantCanonical = `{"event_name":"post_tool_use","hooks":[{"async":false,"command":"sh -c 'if ! command -v entire >/dev/null 2>&1; then exit 0; fi; exec trace hooks codex post-tool-use'","timeout":30,"type":"command"}]}`
	...
	got := codexCommandHookHash(
		"post_tool_use",
		nil, // no matcher
		`sh -c 'if ! command -v entire >/dev/null 2>&1; then exit 0; fi; exec trace hooks codex post-tool-use'`,
		30,
		false,
...
func TestCodexHookTrustState_GeneratesEntriesForEveryDeclaredHandler(t *testing.T) {
  ...
  "hooks": {
    "SessionStart": [
      {"matcher": null, "hooks": [{"type": "command", "command": "trace hooks codex session-start", "timeout": 30}]}
    ],
    "Stop": [
      {"matcher": null, "hooks": [{"type": "command", "command": "trace hooks codex stop", "timeout": 30}]}
    ]
  }
}
`
