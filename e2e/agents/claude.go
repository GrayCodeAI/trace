...
// test runner) and TRACE_TEST_TTY (so agents exercise the real TTY detection
// paths instead of the test override).
func cleanEnv() []string {
	var env []string
	for _, e := range os.Environ() {
		if strings.HasPrefix(e, "CLAUDECODE=") || strings.HasPrefix(e, "TRACE_TEST_TTY=") {
			continue
		}
		env = append(env, e)
	}
...
func (c *Claude) StartSession(ctx context.Context, dir string) (Session, error) {
	...
	s, err := NewTmuxSession(name, dir, []string{"CLAUDECODE", "TRACE_TEST_TTY"}, args[0], args[1:]...)
	if err != nil {
