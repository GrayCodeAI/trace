...
func (g *Gemini) StartSession(_ context.Context, dir string) (Session, error) {
	...
	s, err := NewTmuxSession(name, dir, []string{"CI", "GITHUB_ACTIONS", "TRACE_TEST_TTY", "HOME"}, args[0], args[1:]...)
	if err != nil {
		return nil, err
	}
...
func geminiPromptEnv(repoDir string) []string {
	return append(
		filterEnv(os.Environ(), "TRACE_TEST_TTY", geminiTrustWorkspaceEnvKey),
		"ACCESSIBLE=1",
