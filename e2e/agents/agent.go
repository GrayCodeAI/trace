...
// external agent protocol (trace-agent-* binaries). SetupRepo uses this
// to pre-configure external_agents in settings before running `trace enable`.
...
// removed. Used to strip test-only overrides (e.g. TRACE_TEST_TTY) from agent
