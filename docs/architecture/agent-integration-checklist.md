...
This document provides requirements and a checklist for integrating new AI coding agents with Trace. Use this to validate your implementation is correct and complete.
...
Trace stores the **complete session transcript** at every checkpoint, not incremental diffs. This enables:
...
- Parse the native format when CLI features need specific data (e.g., extract file paths for `trace status`)
