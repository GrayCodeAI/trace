...
The Trace CLI uses **inversion of control** for agent integration: agents are passive data providers that translate their native hook payloads into normalized lifecycle events, and the framework handles all orchestration (state transitions, file detection, checkpoint saving, metadata generation). The flow is:
...
| `HookHandler` | `GetHookNames` | **Required for CLI hook registration** — `trace hooks <agent> <verb>` subcommands are only created for agents implementing this interface. Typically delegates to `HookNames()`. |
...
If your agent uses a JSON config file for hooks (like Claude Code's `.claude/settings.json`, Gemini's `.gemini/settings.json`, Cursor's `.cursor/hooks.json`, Factory AI Droid's `.factory/settings.json`, or Copilot CLI's `.github/hooks/trace.json`), implement `HookSupport`:
...
Also implement `HookHandler` — this is required for the CLI to register `trace hooks <agent> <verb>` subcommands:
...
**What it enables:** `trace enable` automatically installs hooks into the agent's config file.

**Without it:** Users must manually configure hooks to call `trace hooks <agent> <verb>`.

**Implement when:** Your agent supports a config file with hook definitions (e.g., `.claude/settings.json`, `.gemini/settings.json`).

### `HookResponseWriter`

**What it enables:** Displaying messages to the user during hook execution (e.g., the "Trace CLI will link this conversation..." banner on session start).
...
Key difference: OpenCode stores transcripts in a database, not files. The transcript is fetched via `opencode export <sessionID>` at turn-end and cached to `.trace/tmp/<sessionID>.json`.
...
3. **Remove existing Trace hooks** (for `force` mode or mode-switching)
...
- **Idempotent installs** - running `trace enable` twice doesn't duplicate hooks
...
- **Identify Trace hooks** by command prefix (e.g., `"trace "` or `go run "$(git rev-parse --show-toplevel)"/...`)
...
{
  "hooks": {
    "SessionStart": [{"matcher": "", "hooks": [{"type": "command", "command": "trace hooks claude-code session-start"}]}],
    "Stop": [{"matcher": "", "hooks": [{"type": "command", "command": "trace hooks claude-code stop"}]}],
    "PreToolUse": [{"matcher": "Task", "hooks": [{"type": "command", "command": "trace hooks claude-code pre-task"}]}]
  }
}
```
...
```json
{
  ...
  "hooks": {
    "SessionStart": [{"hooks": [{"name": "trace-session-start", "type": "command", "command": "trace hooks gemini session-start"}]}],
    "AfterAgent": [{"hooks": [{"name": "trace-after-agent", "type": "command", "command": "trace hooks gemini after-agent"}]}]
  }
}
```
...
```json
{
  "version": 1,
  "hooks": {
    "sessionStart": [{"command": "trace hooks cursor session-start"}],
    "beforeSubmitPrompt": [{"command": "trace hooks cursor before-submit-prompt"}],
    "stop": [{"command": "trace hooks cursor stop"}],
    "sessionEnd": [{"command": "trace hooks cursor session-end"}],
    "preCompact": [{"command": "trace hooks cursor pre-compact"}],
    "subagentStart": [{"command": "trace hooks cursor subagent-start"}],
    "subagentStop": [{"command": "trace hooks cursor subagent-stop"}]
  }
}
```
...
```json
{
  "hooks": {
    "SessionStart": [
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid session-start"}]},
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid user-prompt-submit"}]}
    ],
    "SessionEnd": [
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid session-end"}]}
    ],
    "UserPromptSubmit": [
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid user-prompt-submit"}]}
    ],
    "Stop": [
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid stop"}]}
    ],
    "PreToolUse": [
      {"matcher": "Task", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid pre-tool-use"}]}
    ],
    "PostToolUse": [
      {"matcher": "Task", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid post-tool-use"}]}
    ],
    "PreCompact": [
      {"matcher": "", "hooks": [{"type": "command", "command": "trace hooks factoryai-droid pre-compact"}]}
    ]
  }
}
```
...
OpenCode uses a TypeScript plugin file (`.opencode/plugins/trace.ts`) instead of a JSON config. The plugin is auto-generated by `trace enable --agent opencode` and uses OpenCode's Bun-based plugin API.
...
- Idempotency via marker string check: `"Auto-generated by \`trace enable --agent opencode\`"`
...
The strings returned by `HookNames()` become literal CLI subcommands under `trace hooks <agent>`. For example, if `HookNames()` returns `["session-start", "stop"]`, the CLI creates:
- `trace hooks your-agent session-start`
- `trace hooks your-agent stop`
