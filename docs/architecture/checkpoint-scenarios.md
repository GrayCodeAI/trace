...
The system uses:
- **Shadow branches** (`trace/<commit-hash>-<worktree-hash>`) - temporary storage for checkpoint data
...
sequenceDiagram
    ...
    G->>SB: Condense to trace/checkpoints/v1
...
- PrepareCommitMsg adds `Trace-Checkpoint` trailer
...
sequenceDiagram
    ...
    Note over G: No Trace-Checkpoint trailer added
```
...
Each checkpoint's `metadata.json` contains cumulative token usage for the trace session up to that point. Summing token counts across multiple checkpoints from the same session **double-counts tokens**.
...
For anything that slips through, run `trace clean --all` manually:

```bash
trace clean --all          # Preview orphaned items
trace clean --all --force  # Delete orphaned items
```
