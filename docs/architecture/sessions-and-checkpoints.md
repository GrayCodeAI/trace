...
Trace CLI creates checkpoints for AI coding sessions. The system is agent-agnostic - it works with Claude Code, Codex, Gemini CLI, OpenCode, Cursor, Factory AI Droid, Copilot CLI, or any tool that triggers Trace hooks.
...
const (
    ...
    Committed             // Metadata + commit ref, trace/checkpoints/v1
)
```
...
```go
// ListSessions returns all sessions from trace/checkpoints/v1,
...
type Store interface {
    ...
    // Committed checkpoint operations (trace/checkpoints/v1 branch - metadata only)
...
|------|----------|----------|
| Session State | `.git/trace-sessions/<id>.json` | Active session tracking |
| Temporary | `trace/<commit[:7]>-<worktreeHash[:6]>` branch | Full state (code + metadata) |
| Committed | `trace/checkpoints/v1` branch (sharded) | Metadata + commit reference |

### Session State

Location: `.git/trace-sessions/<session-id>.json`

Stored in git common dir (shared across worktrees). Tracks active session info.

### Temporary Checkpoints

Branch: `trace/<commit[:7]>-<worktreeHash[:6]>`
...
- Deleted after condensation to `trace/checkpoints/v1`
- Reset if orphaned (no session state file exists)

### Committed Checkpoints

Branch: `trace/checkpoints/v1`
...
1. **User commit trailer**:
   - `Trace-Checkpoint: a3b2c4d5e6f7` added to user's commit message
   - Added by `prepare-commit-msg` hook (user can remove)

2. **Directory sharding** on `trace/checkpoints/v1`:
   - Path: `<id[:2]>/<id[2:]>/` (e.g., `a3/b2c4d5e6f7/`)
   - First 2 chars = shard (256 possible shards)
   - Remaining 10 chars = directory name

3. **Commit subject** on `trace/checkpoints/v1`:
   - Format: `Checkpoint: a3b2c4d5e6f7`
   - Makes `git log trace/checkpoints/v1` readable

**Bidirectional Lookup:**

```
User commit → Metadata:
  1. Extract "Trace-Checkpoint: a3b2c4d5e6f7" from commit message
  2. Read trace/checkpoints/v1 tree at a3/b2c4d5e6f7/

Metadata → User commits:
  Given checkpoint ID a3b2c4d5e6f7
  → Search branch history for commits with "Trace-Checkpoint: a3b2c4d5e6f7"
```

Note: Commit subjects on `trace/checkpoints/v1` (e.g., `Checkpoint: a3b2c4d5e6f7`)
...
└──────────────────────────────────────────────────┘
                  ...
                  post-commit hook runs
                           ↓
          Condense shadow → trace/checkpoints/v1
                           ↓
┌──────────────────────────────────────────────────┐
│ Commit on trace/checkpoints/v1:                 │
...
session/
├── state.go             # Active session state (StateStore, .git/trace-sessions/)
...
- Action: branch renamed from `trace/<old-commit[:7]>-<worktreeHash[:6]>` to `trace/<new-commit[:7]>-<worktreeHash[:6]>`
