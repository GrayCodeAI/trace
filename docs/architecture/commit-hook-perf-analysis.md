...
Measured on a full-history single-branch clone of `GrayCodeAI/cli` with 200 seeded branches and packed refs.
...
production distribution observed in `.git/trace-sessions/`.
...
Control commit (no Trace) is ~25-30ms regardless of session count.
...
that skipped the trace hot path.
...
5. **Condensation**: Read shadow branch data, write to `trace/checkpoints/v1` branch (~30-50ms)
...
`StateStore.List()` does `os.ReadDir()` + `Load()` for every `.json` file in `.git/trace-sessions/`. Each `Load()` reads a file, parses JSON, runs `NormalizeAfterLoad()`, and checks staleness. Called once per hook via `listAllSessionStates()` → `findSessionsForWorktree()`.
...
- Builds a new tree on `trace/checkpoints/v1`
...
1. **Batch condensation**: Instead of condensing sessions one-by-one (each creating a separate commit on `trace/checkpoints/v1`), batch all sessions into a single commit. This would reduce N commits to 1 commit.
