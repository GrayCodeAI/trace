...
The Trace CLI supports kubectl-style external commands — standalone binaries on `$PATH` that extend the CLI without modifying the main repository. When the user invokes `trace <name>` and `<name>` isn't a built-in subcommand, the CLI looks for an `trace-<name>` binary on `$PATH` and execs it with the remaining arguments. Stdio passes through, exit codes propagate, and the parent CLI does no further processing of the child's output.
...
The CLI does not scan `$PATH` at startup. Resolution is lazy: when `os.Args[1]` doesn't match a built-in subcommand, the CLI calls `exec.LookPath("trace-" + os.Args[1])`. If a binary is found and executable, it runs before Cobra parses arguments.
...
4. **Found-but-not-executable surfaces as a launch error.** If `trace-<name>` exists on `$PATH` but lacks the executable bit, the resolver reports `Failed to run plugin trace-<name>` with exit code 1, rather than falling through to Cobra's "unknown command" path.
...
- **Default:** `$XDG_DATA_HOME/trace/plugins/bin` (Linux/macOS) or `%LOCALAPPDATA%\entire\plugins\bin` (Windows).
- **Override:** `$TRACE_PLUGIN_DIR/bin`.
...
`trace plugin install/list/remove` manage the contents of this directory. Authors who prefer the raw "drop a binary on `$PATH`" model don't need to use it.

> **Compatibility note:** the `trace plugin` command group is itself a built-in. Per the "built-ins win" rule above, it shadows any external command named `entire-plugin` that may have existed on `$PATH` previously. The collision is intentional — managing plugins is a built-in concern — but worth flagging for anyone who shipped an `entire-plugin` external command before this layer landed.
...
|---|---|
| `TRACE_CLI_VERSION` | The CLI's version string (e.g. `0.42.0`, `dev`) |
| `TRACE_REPO_ROOT` | Absolute path to the git repository root, when the CLI is invoked inside one. Omitted otherwise. |
| `TRACE_PLUGIN_DATA_DIR` | Per-plugin durable storage directory (`<plugin-root>/data/<name>`). Not pre-created — the plugin should `mkdir -p` on first write. Set regardless of whether the plugin is on raw `$PATH` or in the managed dir, so plugins get the same contract either way. Omitted only in degenerate environments where the per-user data root cannot be resolved (e.g. no home dir, no `LOCALAPPDATA`/`XDG_DATA_HOME`/`TRACE_PLUGIN_DIR`); the parent CLI prints a warning to stderr in that case. |
...
If a plugin needs an environment variable that isn't on the allowlist (for example `AWS_PROFILE` for an `trace-deploy` command), the user can opt names back in via `TRACE_PLUGIN_ENV`. It's a comma-separated list of either exact names or `PREFIX_*` wildcards:

```sh
# Forward AWS_* and EDITOR
TRACE_PLUGIN_ENV='AWS_*,EDITOR' trace deploy

# Forward a single token
TRACE_PLUGIN_ENV='GH_TOKEN' trace pgr
```

`TRACE_PLUGIN_ENV` itself is forwarded to plugins (it matches the `TRACE_` prefix), so plugins can introspect what was opened up.
...
Plugin authors who need a variable should either rely on the allowlist or document the `TRACE_PLUGIN_ENV` value users should set.
...
- **Exit codes propagate verbatim.** The parent `trace` exits with the child's exit code.
...
- **Arguments after the command name pass through verbatim.** `trace pgr --help foo` invokes `trace-pgr` with argv `["--help", "foo"]`. Cobra's flag parsing does not run.
...
- **No checkpoint integration.** File modifications are not tracked in checkpoints. External commands do not appear in `trace activity`. If a tool needs to participate in the session/checkpoint lifecycle, it must use the [agent protocol](external-agent-protocol.md) instead.
- **No transcript recording.** External-command stdio is not captured.
- **No hook installation.** External commands cannot register git hooks or agent hooks via the resolver. They are free to install their own, but `trace` does not coordinate.
...
2. `trace` settings have `Telemetry: true`.
3. `TRACE_TELEMETRY_OPTOUT` is unset.
...
|---|---|---|
| **Binary name pattern** | `trace-<name>` | `trace-agent-<name>` |
...
| **Versioning** | None | `TRACE_PROTOCOL_VERSION` envelope |
