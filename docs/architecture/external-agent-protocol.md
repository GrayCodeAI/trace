...
The Trace CLI supports external agent plugins — standalone binaries that implement the Agent interface via a subcommand-based protocol over stdin/stdout. This allows third-party agents to integrate with the CLI without modifying the main repository.

## Discovery

The CLI discovers external agents by scanning `$PATH` for executables matching the pattern `trace-agent-<name>`. For example, `trace-agent-cursor` would register as the "cursor" agent.
...
|---|---|
| `TRACE_REPO_ROOT` | Absolute path to the git repository root |
| `TRACE_PROTOCOL_VERSION` | Protocol version (`1`) |
...
Installs agent hooks for Trace integration.
...
Those bytes must be the complete cumulative compact **Trace Transcript Format** JSONL for the session up to the current point in time (not a delta/scoped slice since the previous checkpoint).

The CLI writes these bytes byte-for-byte as the full `transcript.jsonl` file for the checkpoint, so external agents must always return the trace compact transcript accumulated so far.
...
summary providers for `trace explain --generate` and checkpoint auto-summarize.
Configure them with:

```bash
trace configure --summarize-provider <agent-name> [--summarize-model <model>]
```

The provider name is derived from the executable name `trace-agent-<agent-name>`.
The CLI passes Trace's summary prompt on stdin to `generate-text --model <model>`
...
The protocol version is declared in the `info` response (`protocol_version` field) and set via `TRACE_PROTOCOL_VERSION` environment variable. The CLI checks that the binary's protocol version matches its expected version before registering the agent.
