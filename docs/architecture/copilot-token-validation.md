...
- `session.shutdown.data.modelMetrics[*].usage`: aggregate for the trace session

That means Trace must treat the same transcript in two different ways:

- Checkpoint metadata must stay scoped to `CheckpointTranscriptStart`
- Session state for `trace status` should use the full-session aggregate once `session.shutdown` exists
...
- Checkpoint metadata: `trace/checkpoints/v1:<checkpoint-shard>/0/metadata.json`
- Session state: `.git/trace-sessions/<session-id>.json`

`trace explain` and `trace status` are useful views, but the raw metadata is the durable source of truth.
...
- Runs the real `trace hooks copilot-cli ...` commands
- Creates two commits with real Trace trailers and condensation
...
- Trace is enabled for Copilot CLI in the test repo
...
git config user.email "trace@graycode.ai"
printf 'start\n' > README.md
git add README.md
git commit -m "init"
trace enable --agent copilot-cli
```
...
```bash
CP1="$(git log -1 --format=%B | sed -n 's/^Trace-Checkpoint: //p' | tail -n1)"
trace explain --checkpoint "$CP1" --no-pager
```
...
```bash
CP2="$(git log -1 --format=%B | sed -n 's/^Trace-Checkpoint: //p' | tail -n1)"
trace explain --checkpoint "$CP2" --no-pager
```
...
### 6. Verify Trace Session Status Uses the Full-Session Total
...
- Checkpoint 2 should not equal `SESSION_TOTAL` unless the second checkpoint genuinely covers the trace session

## Expected Outcomes

The fix is behaving correctly when all of these are true:

- `trace status` reflects the final Copilot `session.shutdown` aggregate
- `trace explain --checkpoint "$CP1"` stays stable before and after turn 2 / session shutdown
