...
This document describes known limitations of the Trace CLI.

## Git Operations

### Amending Commits with `-m` Flag

When you amend a commit using `git commit --amend -m "new message"`, the `-m` flag replaces the trace message including any `Trace-Checkpoint` trailer. Git passes `source="message"` (not `"commit"`) to the prepare-commit-msg hook, so the amend-specific trailer preservation logic is bypassed.
...
**Tracked in:** [ENT-161](https://linear.app/GrayCodeAI/issue/ENT-161)
...
**Tracked in:** [ENT-241](https://linear.app/GrayCodeAI/issue/ENT-241)
...
**Impact:** Cosmetic — extra metadata entries on `trace/checkpoints/v1` with minimal content. No data loss or corruption.
