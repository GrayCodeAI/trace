...
func TestShadowBranchNameForCommit(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			baseCommit: "abc",
			worktreeID: "wt",
			want:       "trace/abc-" + HashWorktreeID("wt"),
		},
	}
...
func TestParseShadowBranchName(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			branchName:   "trace/abc1234-e3b0c4",
			wantCommit:   "abc1234",
			wantWorktree: "e3b0c4",
			wantOK:       true,
		},
		...
		},
		{
			...
			branchName:   "trace/abcdef1234567890-fedcba",
		...
		{
			name:         "trace/checkpoints/v1 is not a shadow branch",
