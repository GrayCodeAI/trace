...
const (
	productionHookWrapperPrefix              = `sh -c 'if ! command -v entire >/dev/null 2>&1; then `
	windowsProductionHookWrapperPrefix       = `where.exe entire >nul 2>nul & if errorlevel 1 `
	nestedWindowsProductionHookWrapperPrefix = `cmd.exe /d /s /c "where.exe entire >nul 2>nul & if errorlevel 1 `
)
