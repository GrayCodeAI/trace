...
func TestInstallHooks_FreshInstall(t *testing.T) {
	...
	// Verify commands
	assertEntryCommand(t, hooksFile.Hooks.Stop, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor stop"))
	assertEntryCommand(t, hooksFile.Hooks.SessionStart, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor session-start"))
	assertEntryCommand(t, hooksFile.Hooks.BeforeSubmitPrompt, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor before-submit-prompt"))
	assertEntryCommand(t, hooksFile.Hooks.PreCompact, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor pre-compact"))
	assertEntryCommand(t, hooksFile.Hooks.SubagentStart, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor subagent-start"))
	assertEntryCommand(t, hooksFile.Hooks.SubagentStop, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor subagent-stop"))
}
...
func TestInstallHooks_PreservesExistingHooks(t *testing.T) {
	...
	// Stop should have user hook + trace hook
	if len(hooksFile.Hooks.Stop) != 2 {
		t.Errorf("Stop hooks = %d, want 2 (user + trace)", len(hooksFile.Hooks.Stop))
	}
	assertEntryCommand(t, hooksFile.Hooks.Stop, "echo user hook")
	assertEntryCommand(t, hooksFile.Hooks.Stop, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor stop"))

	// SubagentStop should have user Write hook + Trace hook
	if len(hooksFile.Hooks.SubagentStop) != 2 {
		t.Errorf("SubagentStop hooks = %d, want 2 (user Write + Trace)", len(hooksFile.Hooks.SubagentStop))
	}
...
func TestInstallHooks_LocalDev(t *testing.T) {
	...
	hooksFile := readHooksFile(t, tempDir)
	assertEntryCommand(t, hooksFile.Hooks.Stop, `go run "$(git rev-parse --show-toplevel)"/cmd/hawk trace hooks cursor stop`)
}
...
func TestInstallHooks_PreservesUnknownFields(t *testing.T) {
  ...
  "hooks": {
	...
	if len(stopHooks) != 2 {
		t.Errorf("stop hooks = %d, want 2 (user + trace)", len(stopHooks))
	}
	assertEntryCommand(t, stopHooks, "echo user stop")
	assertEntryCommand(t, stopHooks, agent.WrapProductionSilentHookCommand("hawk trace hooks cursor stop"))
}
