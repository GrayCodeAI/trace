...
// createCheckpointOnMetadataBranch creates a checkpoint on the trace/checkpoints/v1 branch
...
// createCheckpointOnMetadataBranchFull creates a checkpoint on the trace/checkpoints/v1 branch
...
func TestCheckRemoteMetadata_MetadataExistsOnRemote(t *testing.T) {
	...
	// Create checkpoint metadata on local trace/checkpoints/v1 branch
	sessionID := "2025-01-01-test-session"
	checkpointID := createCheckpointOnMetadataBranch(t, repo, sessionID)

	// Copy the local trace/checkpoints/v1 to origin/trace/checkpoints/v1 (simulate remote)
	...
	// Delete local trace/checkpoints/v1 branch to simulate "not fetched yet"
...
func TestCheckRemoteMetadata_NoRemoteMetadataBranch(t *testing.T) {
	...
	// Delete local trace/checkpoints/v1 branch
	if err := repo.Storer.RemoveReference(plumbing.NewBranchReferenceName(paths.MetadataBranchName)); err != nil {
		t.Fatalf("Failed to remove local metadata branch: %v", err)
	}
...
func TestResumeFromCurrentBranch_NoMetadataAvailable(t *testing.T) {
	...
	commitMsg := "Add feature\n\nTrace-Checkpoint: " + checkpointID.String()
	var err error
