...
func TestFetchBranchIfMissing_CreatesLocalFromRemote(t *testing.T) {
	...
	// Create an orphan branch in the remote repo (simulating trace/checkpoints/v1)
	cmd := exec.CommandContext(ctx, "git", "checkout", "--orphan", "trace/checkpoints/v1")
	cmd.Dir = remoteDir
	...
	assert.False(t, testutil.BranchExists(t, localDir, "trace/checkpoints/v1"))
	...
	assert.True(t, testutil.BranchExists(t, localDir, "trace/checkpoints/v1"))
}
...
func TestFetchMetadataBranch_FetchesAndCreatesLocalBranch(t *testing.T) {
	...
	// Set up a "remote" repo with trace/checkpoints/v1
	remoteDir := t.TempDir()
	testutil.InitRepo(t, remoteDir)
	testutil.WriteFile(t, remoteDir, "f.txt", "init")
	testutil.GitAdd(t, remoteDir, "f.txt")
	...
	assert.False(t, testutil.BranchExists(t, localDir, "refs/trace-fetch-tmp/trace/checkpoints/v1"))
}
...
func TestFetchMetadataBranch_UpdatesExistingLocalBranch(t *testing.T) {
	...
	// Record initial hash
	hashCmd := exec.CommandContext(ctx, "git", "rev-parse", "trace/checkpoints/v1")
	...
	cmd = exec.CommandContext(ctx, "git", "checkout", "trace/checkpoints/v1")
	cmd.Dir = remoteDir
	cmd.Env = testutil.GitIsolatedEnv()
	require.NoError(t, cmd.Run())
...
// FetchMetadataBranch with a remote whose trace/checkpoints/v1 is at commit A
