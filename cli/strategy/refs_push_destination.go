...
func resolveRefsPushDestination(ctx context.Context, ps pushSettings) refsPushDestination {
	...
	if err != nil {
		...
		logging.Debug(
			ctx, "git-refs push: could not enumerate push URLs; using target as given",
...
func (d refsPushDestination) warnIgnoredPushURLs(ctx context.Context) {
	...
	logging.Info(
		ctx, "git-refs push: multi-URL remote, pushing checkpoint refs to the first push URL only",
