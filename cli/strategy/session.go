...
// ListSessions returns all sessions from the trace/checkpoints/v1 branch,
...
func ListSessions(ctx context.Context) ([]Session, error) {
	...
	// Get checkpoints from the trace/checkpoints/v1 branch
	...
	if additionalSessions, err := start.GetAdditionalSessions(ctx); err == nil {
...
// getDescriptionForCheckpoint reads the description for a checkpoint from the trace/checkpoints/v1 branch.
