...
func (s *ManualCommitStrategy) finalizeAllTurnCheckpoints(ctx context.Context, state *SessionState) int {
	...
	if settings.IsImageExternalizationEnabled(ctx) {
		...
		if exErr != nil {
			logging.Warn(
				logCtx, "finalize: image externalization failed; leaving transcript inline",
				slog.String("session_id", state.SessionID),
	...
	if err != nil {
		logging.Warn(
			logCtx, "finalize: failed to open checkpoint store",
			slog.String("error", err.Error()),
		)
		return 1 // Count as error - all checkpoints will be skipped
	}
...
func (s *ManualCommitStrategy) carryForwardToNewShadowBranch(
	...
	if err != nil {
		logging.Warn(
			logCtx, "post-commit: carry-forward failed to open checkpoint store",
			slog.String("session_id", state.SessionID),
			slog.String("error", err.Error()),
		)
		return
