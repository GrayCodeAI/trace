...
func doPushBranch(ctx context.Context, target, branchName string) error {
	...
	fmt.Fprintf(os.Stderr, "[trace] Pushing %s to %s...", branchName, displayTarget)
	...
	fmt.Fprintf(os.Stderr, "[trace] Syncing %s with remote...", branchName)
	stop = startProgressDots(os.Stderr)
...
func printCheckpointRemoteHint(target string) {
	...
	fmt.Fprintln(os.Stderr, "[trace] A checkpoint remote is configured in Trace settings (.trace/settings.json or .trace/settings.local.json) but could not be reached.")
...
func printCheckpointsV2MigrationHint(ctx context.Context) {
	checkpointsV2MigrationHintOnce.Do(func() {
		...
		fmt.Fprintln(os.Stderr, "[trace] Note: .trace/settings.json sets checkpoints_version: 2. Run 'trace migrate --checkpoints v2' to migrate existing checkpoints to v2.")
		fmt.Fprintln(os.Stderr, "[trace] Use 'trace migrate --checkpoints v2 --force' to rewrite all checkpoints in v2.")
	})
}
...
// isCheckpointsVersion2Committed returns true if the committed .trace/settings.json
...
func fetchAndRebaseSessionsCommon(ctx context.Context, target, branchName string) error {
	...
	if usedTempRef {
		tmpRef := "refs/trace-fetch-tmp/" + branchName
...
func collectCommitsSince(ctx context.Context, repo *git.Repository, repoPath string, tip, exclude plumbing.Hash) ([]*object.Commit, error) {
	...
	cmd := exec.CommandContext(ctx, "git", "rev-list", "--reverse", "--topo-order", "--no-merges", exclude.String()+".."+tip.String()) // #nosec G204 -- fixed "git" binary; exclude/tip are internally resolved commit hashes, not remote input
	cmd.Dir = repoPath
	output, err := cmd.Output()
	if err != nil {
