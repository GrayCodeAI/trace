...
// to the branch's trail as a finding (`trace trail finding`).
const (
...
func selectReviewProfile(s *settings.EntireSettings, override string) (string, settings.ReviewProfileConfig, error) {
	...
	if s == nil || len(s.ReviewProfiles) == 0 {
		return "", settings.ReviewProfileConfig{}, errors.New("no review profiles configured; run `trace review --configure` or add review_profiles to Entire preferences")
	}
...
func defaultReviewProfileForInstalledAgents(
	...
	if len(agents) == 0 {
		return settings.ReviewProfileConfig{}, errors.New("no agents with review runner adapters and hooks installed; run `trace configure --agent claude-code`, `trace configure --agent codex`, `trace configure --agent gemini`, or `trace configure --agent pi`")
	}
