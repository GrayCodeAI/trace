...
This repo contains the CLI for Trace.
...
- `trace/`: Main CLI entry point
- `trace/cli`: CLI utilities and helpers (Cobra commands, helpers, group roots)
- `trace/cli/commands`: actual command implementations
- `trace/cli/agent`: agent implementations (Claude Code, Gemini CLI, OpenCode, Cursor, Factory AI Droid, Copilot CLI) - see [Agent Integration Checklist](docs/architecture/agent-integration-checklist.md) and [Agent Implementation Guide](docs/architecture/agent-guide.md)
- `trace/cli/strategy`: strategy implementation (manual-commit) - see section below
- `trace/cli/checkpoint`: checkpoint storage abstractions (temporary and committed)
- `trace/cli/session`: session state management
- `trace/cli/integration_test`: integration tests (simulated hooks)
...
- `configure`: bare prints help and a hint pointing at `trace agent`; flags
  manage non-agent settings (telemetry, git-hook installation mode, strategy
  options, summary provider). Agent CRUD lives under `trace agent`.
...
Tests that spawn the real `trace` or `git` binary need the child to be non-interactive so prompts don't hang on a developer terminal.

`interactive.CanPromptInteractively()` resolves in this order:

1. `TRACE_TEST_TTY=1` → force interactive ON (any other non-empty value → force OFF).
2. `testing.Testing()` → false. In-process `go test` runs are non-interactive by default; no per-test `t.Setenv("TRACE_TEST_TTY", "0")` is needed.
...
`interactive.UnderTest()` returns true when `testing.Testing()` or `TRACE_TEST_TTY` is set — use it where code needs to skip a real-terminal operation even if `CanPromptInteractively()` returns true (e.g., reading from `/dev/tty` directly inside `askConfirmTTY`).
...
if _, err := paths.WorktreeRoot(); err != nil {
    ...
    fmt.Fprintln(cmd.ErrOrStderr(), "Not a git repository. Please run 'trace enable' from within a git repository.")
...
- Read `.trace/settings.json` or `.trace/settings.local.json` directly with `os.ReadFile`
...
- `settings/settings.go` - `TraceSettings` struct, `Load()`, and helper methods
...
go-git v5 has a bug where `worktree.Reset()` with `git.HardReset` and `worktree.Checkout()` incorrectly delete untracked directories even when they're listed in `.gitignore`. This would destroy `.trace/` and `.worktrees/` directories.
...
- Creates shadow branch `trace/<HEAD-commit-hash[:7]>-<worktreeHash[:6]>` per base commit + worktree
...
- Condenses session logs to permanent `trace/checkpoints/v1` branch on user commits
...
- **Copilot token scoping** - Copilot CLI `session.shutdown` contains session-wide token aggregates. Checkpoint metadata must stay scoped to `CheckpointTranscriptStart`; condensation may separately backfill full-session Copilot totals into session state for `trace status`.
- Tracks session state in `.git/trace-sessions/` (shared across worktrees)
- **Shadow branch migration** - if user does stash/pull/rebase (HEAD changes without commit), shadow branch is automatically moved to new base commit
- **Orphaned branch cleanup** - if a shadow branch exists without a corresponding session state file, it is automatically reset when a new session starts
- PrePush hook can push `trace/checkpoints/v1` branch alongside user pushes
...
- `push_common.go` - PrePush logic for pushing `trace/checkpoints/v1` branch
...
- `manual_commit_condensation.go` - Condense logic for copying logs to `trace/checkpoints/v1`
...
**Shadow branches** (`trace/<commit-hash[:7]>-<worktreeHash[:6]>`):

```
.trace/metadata/<session-id>/
...
**Metadata branch** (`trace/checkpoints/v1`) - sharded checkpoint format:
...
**Session State** (filesystem, `.git/trace-sessions/`):
...
2. **Added to user commits** via `Trace-Checkpoint` trailer:
   - **Manual-commit**: Added via `prepare-commit-msg` hook (user can remove it before committing)

3. **Used for directory sharding** on `trace/checkpoints/v1` branch:
   - Path format: `<id[:2]>/<id[2:]>/`
   - Example: `a3b2c4d5e6f7` → `a3/b2c4d5e6f7/`
   - Creates 256 shards to avoid directory bloat

4. **Appears in commit subject** on `trace/checkpoints/v1` commits:
   - Format: `Checkpoint: a3b2c4d5e6f7`
   - Makes `git log trace/checkpoints/v1` readable and searchable

**Bidirectional linking:**

```
User commit → Metadata:
  Extract "Trace-Checkpoint: a3b2c4d5e6f7" trailer
  → Read a3/b2c4d5e6f7/ directory from trace/checkpoints/v1 tree at HEAD

Metadata → User commits:
  Given checkpoint ID a3b2c4d5e6f7
  → Search user branch history for commits with "Trace-Checkpoint: a3b2c4d5e6f7" trailer
```

Note: Commit subjects on `trace/checkpoints/v1` (e.g., `Checkpoint: a3b2c4d5e6f7`) are
...
User's commit (on main branch):
  ...
  Trace-Checkpoint: a3b2c4d5e6f7"
       ↓ ↑
       Linked via checkpoint ID
       ↓ ↑
trace/checkpoints/v1 commit:
...
- `Trace-Checkpoint: <checkpoint-id>` - 12-hex-char ID linking to metadata on `trace/checkpoints/v1`
  - Added via `prepare-commit-msg` hook; user can remove it before committing to skip linking

**On shadow branch commits (`trace/<commit-hash[:7]>-<worktreeHash[:6]>`):**

- `Trace-Session: <session-id>` - Session identifier
- `Trace-Metadata: <path>` - Path to metadata directory within the tree
- `Trace-Task-Metadata: <path>` - Path to task metadata directory (for task checkpoints)
- `Trace-Strategy: manual-commit` - Strategy that created the commit

**On metadata branch commits (`trace/checkpoints/v1`):**
...
- `Trace-Session: <session-id>` - Session identifier
- `Trace-Strategy: <strategy>` - Strategy name (manual-commit)
- `Trace-Agent: <agent-name>` - Agent name (optional, e.g., "Claude Code")
- `Ephemeral-branch: <branch>` - Shadow branch name (optional)
- `Trace-Metadata-Task: <path>` - Task metadata path (optional, for task checkpoints)

**Note:** The strategy keeps active branch history clean - the only addition to user commits is the single `Trace-Checkpoint` trailer. It never creates commits on the active branch (the user creates them manually). All detailed session data (transcripts, prompts, context) is stored on the `trace/checkpoints/v1` orphan branch or shadow branches.
...
- Action: shadow branch is renamed from `trace/<old-hash>-<worktreeHash>` to `trace/<new-hash>-<worktreeHash>`
