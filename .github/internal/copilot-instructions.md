# Copilot Instructions for Trace CLI

## Repository Overview

Trace CLI is a Go CLI tool that captures AI coding agent sessions (Claude Code, Gemini CLI, OpenCode, Cursor, Factory AI Droid, Copilot CLI, etc.) as searchable metadata stored separately from code commits. It uses a strategy pattern for session/checkpoint management with minimal impact on commit history.
...
```go
// WRONG - deletes .trace/ and .worktrees/
...
└── cli/              # Main CLI package
    ...
    ├── session/      # Session state management (.git/trace-sessions/)
